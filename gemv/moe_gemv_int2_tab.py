...
memory (perf/gemv/test_gemv_ncontig.py); the byte-row variant of the same idea reached only
157 / 14 GB/s and was rejected.
...
Derived from perf/int2_gemv/moe_gemv_int2_ncontig_v4.py (word-load, CPU-interpreter verified).
"""
import torch
import triton
...
def _moe_gemv_int2_tab(
    a_ptr, stride_am,                 # activations bf16 [M, K]; row r uses a[r // TOP_K]
    wtab_ptr, stab_ptr,               # int64 [E]: base address of qweight / scales per expert
    topk_ids_ptr, topk_w_ptr,         # [R] original ids, [R] routed weights (fp32)
    c_ptr, stride_cm,                 # out bf16 [R, N]
    N, K, stride_ww, stride_sg,       # row strides: qweight words per K/16 row, scales per group row
    TOP_K: tl.constexpr, GROUP: tl.constexpr,
    ...
    for k0 in range(0, K, BLOCK_K):
        w = tl.load(wbase + (k0 // 16 + offs_w[:, None]) * stride_ww + offs_n[None, :],
                    mask=n_mask[None, :], other=0)                              # [NW, BN] int32
        part = tl.zeros([NW, BLOCK_N], dtype=tl.float32)
        for i in tl.static_range(16):
            x_i = tl.load(a_base + k0 + offs_w * 16 + i).to(tl.float32)      # [NW], warp-uniform
            part += ((w >> (2 * i)) & 3).to(tl.float32) * x_i[:, None]
        xs = tl.sum(tl.load(a_base + k0 + tl.arange(0, BLOCK_K)).to(tl.float32)
                    .reshape([NG, GROUP]), axis=1)                             # [NG] zero-point term
        part_g = tl.sum(tl.reshape(part, [NG, GROUP // 16, BLOCK_N]), axis=1)  # [NG, BN]
        s = tl.load(sbase + (k0 // GROUP + tl.arange(0, NG)[:, None]) * stride_sg + offs_n[None, :],
...
def moe_gemv_int2_tab(a, wtab, stab, topk_ids, topk_w, N, K, stride_ww, stride_sg,
                      top_k, mul_routed_weight, block_n=64, block_k=128, num_warps=4,
                      scale_bf16=True):
    ...
    _moe_gemv_int2_tab[(triton.cdiv(N, block_n), R)](
        a, a.stride(0), wtab, stab, topk_ids, topk_w, c, c.stride(0),
        N, K, stride_ww, stride_sg,
        TOP_K=top_k, GROUP=128, BLOCK_N=block_n, BLOCK_K=block_k,
        MUL_ROUTED_WEIGHT=mul_routed_weight, SCALE_BF16=scale_bf16, num_warps=num_warps)
    return c
...
def make_tables(qweight, scales):
    ...
    assert qweight.dtype == torch.int32 and scales.dtype in (torch.float16, torch.bfloat16), \
        (qweight.dtype, scales.dtype)
    assert qweight.is_contiguous() and scales.is_contiguous()
    E = qweight.shape[0]
    wt = torch.arange(E, dtype=torch.int64) * (qweight.stride(0) * 4) + qweight.data_ptr()

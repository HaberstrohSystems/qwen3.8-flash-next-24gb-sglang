...
class ExpertElastic:
    ...
    def __init__(self, mass: torch.Tensor, S0: int, device="cuda"):
        ...
        self.order = torch.argsort(self.mass, dim=1, descending=True)   # [L, E] rank -> expert
        ...
        self._ctl_mtime = os.stat(self.ctl).st_mtime if self.ctl and os.path.exists(self.ctl) else 0.0   # ignore stale commands
        _af = os.environ.get("SGLANG_MOE_ELASTIC_FILL_MB")               # reserve to keep free; unset = no autofill
        self._autofill = int(_af) if _af else None
        self.regrow_reserve = int(_af) if _af else 768                    # after a forced shrink, grow back to this
    ...
    def _pin_rows(self, name, need, tail, dtype, why):
        ...
        nbytes = need * int(torch.empty(0, dtype=dtype).element_size()) * int(torch.Size(tail).numel())
        if self.pin_used + nbytes > self.pin_budget:
            raise RuntimeError(f"elastic: pinned budget exhausted for {name} ({why}); "
        ...
        logger.info("elastic: pinned %d rows of %s (%.0f MB) [%s]", need, name, nbytes / 1e6, why)

    def _take_slots(self, name, n, tail, dtype):
        pool = self.pool[name]
        if len(pool) < n:
            if self._serving:
                raise RuntimeError(f"elastic: host slot pool for {name} exhausted ({len(pool)} < {n}) "
    ...
    def add_layer(self, layer):
        ...
        for name in NAMES:
            ...
            arena = RowArena(rb, E, torch.device(self.device).index or 0, name=f"L{lid}.{name}")
            arena.ensure_rows(S)
            ...
            if t.is_cuda:
                ...
                for i, e in enumerate(cold.tolist()):
                    dst, r = slots[i]
                    dst[r].copy_(cold_cpu[i]); home[e] = slots[i]
                    tab[e] = self._slot_addr(slots[i], rb); keep.append(dst)
                del cold_cpu
            else:
                src_tab = (t.data_ptr() + torch.arange(E, dtype=torch.int64) * rb).to(self.device)
                _gather(src_tab, hot.to(self.device), view.view(torch.uint8).view(S, -1), rb)
                for e in cold.tolist():
                    home[e] = (t, e); tab[e] = t.data_ptr() + e * rb
    ...
    def place_all(self, layers):
        ...
        def need(l):
            return {n: (self.E - self.S0 if getattr(l, n).data.is_cuda else 0) for n in NAMES if getattr(l, n, None) is not None}
        donors = [l for l in layers if not any(need(l).values())]
        takers = [l for l in layers if any(need(l).values())]
        placed = 0
        while donors or takers:
            ready = next((l for l in takers if all(len(self.pool[n]) >= k for n, k in need(l).items())), None)
            if ready is not None:
                takers.remove(ready); self.add_layer(ready)
            elif donors:
                self.add_layer(donors.pop(0))
            else:                                        # pool short: pinned fallback (or a clear error)
                self.add_layer(takers.pop(0))
            placed += 1
        if self.reserve_rows:                        # one-time reserve so shrinking below S0 never pins at runtime
            for name in NAMES:
                k = next((k for st in self.layers.values() for k in st.kinds.values() if k.name == name), None)
                if k is not None:
                    self._pin_rows(name, self.reserve_rows, k.tail, k.dtype, "startup reserve")
        self._serving = True
        logger.info("elastic placement: %d layers at S=%d, free slots %s, VRAM free %.2f GB",
                    placed, self.S0, {n: len(v) for n, v in self.pool.items()},
                    torch.cuda.mem_get_info()[0] / 2**30)
        self.write_status()

    # ------------------------------------------------------------------ resize
    def resize_layer(self, lid, S_new):
        st = self.layers[lid]; S = st.S
        ...
        if S_new > S:
            ...
            torch.cuda.synchronize()
            for k in st.kinds.values():
                k.tab[ids.to(self.device)] = (k.arena.base + torch.arange(S, S_new, dtype=torch.int64) * k.rb).to(self.device)
                for e in ids.tolist():
                    self.pool[k.name].append(k.home[e]); k.home[e] = None
        else:
            ids = order[S_new:S]
            n = int(ids.numel()); taken = {}
            ...
            for k in st.kinds.values():
                ...
                for i, e in enumerate(ids.tolist()):
                    ...
                    k.home[e] = slots[i]; addrs[i] = self._slot_addr(slots[i], k.rb)
                torch.cuda.synchronize()
    ...
    def s_floor(self):
        ...
        if not self.layers:
            return 0
        L = len(self.layers); cur = sum(st.S for st in self.layers.values())
        avail = 0 if self._serving else self.pin_budget - self.pin_used
        rb = {n: next(k.rb for st in self.layers.values() for k in st.kinds.values() if k.name == n)
              for n in NAMES if any(n in st.kinds for st in self.layers.values())}
    ...
    def set_S(self, S):
        ...
        logger.info("elastic: S=%d for all layers in %.2fs, VRAM free %.2f GB", S, time.time() - t0,
                    torch.cuda.mem_get_info()[0] / 2**30)
    ...
    def fill(self, reserve_mb, step=8):
        ...
        while True:
            ...
            if cur >= self.E:
                break
            need = sum(k.arena.bytes_to_reach(min(self.E, st.S + step))
                       for st in self.layers.values() for k in st.kinds.values())
            ...
            try:
                ...
                break
        logger.info("elastic fill: S=%s, VRAM free %.2f GB", sorted({st.S for st in self.layers.values()}),
                    torch.cuda.mem_get_info()[0] / 2**30)
    ...
    def free(self, want_mb, step=8):
        ...
        while torch.cuda.mem_get_info()[0] < want:
            ...
            except (ArenaOOM, RuntimeError) as ex:      # partial shrink is still a shrink; report and stop
                logger.warning("elastic free stopped: %s", ex)
                break
        self.pending_regrow = True
        logger.info("elastic free: S=%s, VRAM free %.2f GB", sorted({st.S for st in self.layers.values()}),
                    torch.cuda.mem_get_info()[0] / 2**30)
    ...
    def write_status(self):
        ...
        arena = sum(k.arena.backed_bytes for st in self.layers.values() for k in st.kinds.values())
        S = [self.layers[l].S for l in sorted(self.layers)]
        covered = sum(float(self.mass[l][self.order[l][:self.layers[l].S]].sum()) for l in self.layers) / float(self.mass.sum())
    ...
    def poll(self, m: int = 0, lid: int = 0):
        ...
        if torch.cuda.is_current_stream_capturing() or lid != 0 or m > 16:
            return
        if self._autofill is not None and self._calls > 48 * 4:          # after warmup: grow into free VRAM
        ...
        try:
            ...
            if cmd[0] == "S":
                ...
                self.free(int(cmd[1]))
            logger.info("elastic ctl '%s' done in %.2fs", " ".join(cmd), time.time() - t0)
